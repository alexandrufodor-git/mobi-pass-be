-- Fix: profiles must be inserted before user_roles.
-- user_roles.user_id has a FK → profiles.user_id, so the profile row
-- must exist first. Swap steps 2 and 3 only — no other changes.
