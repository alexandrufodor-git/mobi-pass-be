...
COMMENT ON TYPE public.contract_status IS
  'Contract signing workflow status. Tracks progression from pending (requested) to final approval. terminated is set manually by HR. declined_by_employee is set via eSignatures webhook.';
...
-- ============================================
-- 4. Recreate update_contract_status()
-- ============================================
...
AS $$
BEGIN
  -- Terminal guard: once in a terminal state, do not auto-overwrite.
  -- terminated is set manually by HR; declined_by_employee is set via webhook.
  IF TG_OP = 'UPDATE'
     AND OLD.contract_status IN (
       'terminated'::public.contract_status,
       'declined_by_employee'::public.contract_status
     ) THEN
    RETURN NEW;
  END IF;
...
-- ============================================
-- 5. Recreate update_bike_benefit_status()
--    Clears contract_declined_at on choose_bike reset
-- ============================================
