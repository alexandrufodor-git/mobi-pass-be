...
BEGIN
  ...
  IF NEW.step = 'choose_bike'::public.bike_benefit_step THEN
    ...
    IF TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND OLD.step IS DISTINCT FROM 'choose_bike') THEN
      ...
      NEW.request_contract_at := NULL;        -- Match the prompt: request_contract_at
      NEW.commited_at := NULL;               -- Match the prompt: commited_at
  ...
  ELSIF NEW.step = 'book_live_test' THEN
    NEW.benefit_status := CASE 
      WHEN NEW.live_test_whatsapp_sent_at IS NOT NULL THEN 'testing' 
      ELSE 'searching' 
    END;
  ELSIF NEW.step = 'commit_to_bike' THEN
    NEW.benefit_status := CASE 
      WHEN NEW.commited_at IS NOT NULL THEN 'active' 
      ELSE 'searching' 
    END;
  ...
  ELSIF NEW.request_contract_at IS NOT NULL THEN
    NEW.contract_status := 'pending'; -- This replaces 'not_started'
  ELSE
